...
class StructuredVespaSchema(VespaSchema):
    ...
    def _generate_document_section(self, schema_name: str) -> (List[str], StructuredMarqoIndex):
        ...
        document: List[str] = [
            f'document {{',
            f'field {common.FIELD_ID} type string {{ indexing: summary }}',
        ]
        ...
        for tensor_field_request in self._index_request.tensor_fields:
            ...
            document.extend(
                (
                    f'field {chunks_field_name} type array<string> {{',
                    'indexing: attribute | summary',
                    '}',
                    ...
                    'index { hnsw {',
                    f'max-links-per-node: {self._index_request.hnsw_config.m}',
    ...
    def _generate_rank_profiles(self, marqo_index: StructuredMarqoIndex) -> List[str]:
        ...
        if lexical_fields:
            rank_profiles.extend(
                (
                    f'rank-profile {common.RANK_PROFILE_BM25} inherits default {{',
                    'inputs {',
                )
            )
            rank_profiles.extend(f'query({field.name}): 0' for field in lexical_fields)
            rank_profiles.extend(
                ('}', 'first-phase {', f'expression: {bm25_expression}', '}}')
            )
        if tensor_fields:
            rank_profiles.extend(
                (
                    f'rank-profile {common.RANK_PROFILE_EMBEDDING_SIMILARITY} inherits default {{',
                    'inputs {',
                    f'query({common.QUERY_INPUT_EMBEDDING}) tensor<float>(x[{model_dim}])',
                )
            )
            rank_profiles.extend(f'query({field.name}): 0' for field in tensor_fields)
            rank_profiles.extend(
                (
                    '}',
                    'first-phase {',
                    f'expression: {embedding_similarity_expression}',
                    '}',
                    embedding_match_features_expression,
                    '}',
                )
        ...
        if score_modifier_fields_names:
            ...
            rank_profiles.extend(
                (
                    f'rank-profile {common.RANK_PROFILE_MODIFIERS} inherits default {{',
                    'inputs {',
                    f'query({common.QUERY_INPUT_SCORE_MODIFIERS_MULT_WEIGHTS}) tensor<float>(p{{}})',
                    f'query({common.QUERY_INPUT_SCORE_MODIFIERS_ADD_WEIGHTS}) tensor<float>(p{{}})',
                    '}',
            ...
            if lexical_fields:
                rank_profiles.extend(
                    (
                        f'rank-profile {common.RANK_PROFILE_BM25_MODIFIERS} inherits {common.RANK_PROFILE_MODIFIERS} {{',
                        'inputs {',
                        f'query({common.QUERY_INPUT_SCORE_MODIFIERS_MULT_WEIGHTS}) tensor<float>(p{{}})',
                        f'query({common.QUERY_INPUT_SCORE_MODIFIERS_ADD_WEIGHTS}) tensor<float>(p{{}})',
                    )
                ...
                rank_profiles.extend(
                    (
                        '}',
                        'first-phase {',
                        f'expression: modify({bm25_expression})',
                        '}}',
                    )
                )
            if tensor_fields:
                rank_profiles.extend(
                    (
                        f'rank-profile {common.RANK_PROFILE_EMBEDDING_SIMILARITY_MODIFIERS} inherits {common.RANK_PROFILE_MODIFIERS} {{',
                        'inputs {',
                        f'query({common.QUERY_INPUT_SCORE_MODIFIERS_MULT_WEIGHTS}) tensor<float>(p{{}})',
                        f'query({common.QUERY_INPUT_SCORE_MODIFIERS_ADD_WEIGHTS}) tensor<float>(p{{}})',
                        f'query({common.QUERY_INPUT_EMBEDDING}) tensor<float>(x[{model_dim}])',
                ...
                rank_profiles.extend(
                    (
                        '}',
                        'first-phase {',
                        f'expression: modify({embedding_similarity_expression})',
                        '}',
                        embedding_match_features_expression,
                        '}',
                    )
    ...
    def _generate_default_fieldset(self, marqo_index: StructuredMarqoIndex) -> List[str]:
        ...
        if fieldset_fields := marqo_index.lexical_field_map.keys():
            fieldsets.extend(
                (
                    'fieldset default {',
                    f'fields: {", ".join(fieldset_fields)}',
                    '}',
                )
            )
        return fieldsets
    ...
    def _generate_summaries(self, marqo_index: StructuredMarqoIndex) -> List[str]:
        ...
        non_vector_summary_fields = [f'summary {common.FIELD_ID} type string {{ }}']
        ...
        summaries.extend(('}', f'document-summary {common.SUMMARY_ALL_VECTOR} {{'))
        summaries.extend(non_vector_summary_fields)
